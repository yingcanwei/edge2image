...
parser.add_argument("--operation", required=True, choices=["grayscale", "resize", "blank", "combine", "edges","all"])
...
parser.add_argument("--resize_dir", type=str, help="path to folder containing B images for combine operation")
parser.add_argument("--gray_dir", type=str, help="path to folder containing B images for combine operation")
...
def resize(src):
    ...
    if height != width:
        if a.pad:
            ...
            oh = (size - height) // 2
            ow = (size - width) // 2
            dst = im.pad(image=dst-255, offset_height=oh, offset_width=ow, target_height=size, target_width=size)+255
        else:
...
if a.operation == "all":
    ...
    a.output_dir = a.resize_dir
    a.operation = "resize"
    main()
    a.input_dir = a.resize_dir
    a.output_dir = a.gray_dir
    a.operation = "grayscale"
    main()
    a.input_dir = a.gray_dir
    a.b_dir = a.gray_dir
